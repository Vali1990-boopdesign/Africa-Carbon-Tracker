...
export async function reimportSpecificCSVData() {
  try {
    console.log("🔄 Starting reimport of latest specific CSV file...");
    ...
    // Import transactions from the latest specific CSV
    ...
    if (!fs.existsSync(transactionsPath)) {
      console.error("❌ Latest specific transactions CSV file not found");
      return;
    }
    ...
    console.log(`\n🎉 Latest specific CSV import completed!`);
    ...
    console.error("❌ Latest specific CSV import failed:", error);
    throw error;
  }
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  reimportSpecificCSVData()
    .then(result => {
      console.log("✅ Specific CSV import successful:", result);
      process.exit(0);
    })
    .catch(error => {
      console.error("❌ Specific CSV import failed:", error);
      process.exit(1);
    });
}

...
export async function importCurrentCSVData() {
  try {
    console.log("🔄 Starting import of current CSV data...");
    ...
    // Import transactions from the latest CSV file
    ...
    if (!fs.existsSync(transactionsPath)) {
      console.error("❌ Current transactions CSV file not found at:", transactionsPath);
      return;
    }
    ...
    console.error("❌ Current CSV import failed:", error);
    throw error;
  }
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  importCurrentCSVData()
    .then(result => {
      console.log("✅ Current CSV import successful:", result);
      process.exit(0);
    })
    .catch(error => {
      console.error("❌ Current CSV import failed:", error);

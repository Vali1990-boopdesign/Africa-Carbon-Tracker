...
The backend is developed with Node.js and Express.js, providing a REST API. Data is persisted using PostgreSQL 16 via Replit Helium (development) with Drizzle ORM. The database connection uses the standard `pg` (node-postgres) driver with conditional SSL (disabled in development, enabled in production). Core data models include Transactions, Buyer Profiles, and Dashboard Metrics. API endpoints serve various data types such as dashboard metrics, transactional data, geographical data, sector breakdowns, time-series data, and top buyer information.

...
export function AfricaChoroplethWithTabs({ 
  ...
  useEffect(() => {
    ...
    // Create data map for fast lookups (following D3 Gallery pattern)
    const dataMap = new Map(data.map(d => [d.country, {
      credits: d.totalCredits,
      projects: d.activeProjects
    }]));

    // Map projection - Mercator centered on Africa (like D3 Gallery example)
    ...
    // Current color scale based on active tab
    const currentColorScale = activeTab === "credits" ? creditsColorScale : projectsColorScale;
    const currentMetric = activeTab === "credits" ? "credits" : "projects";

    // Create tooltip (following D3 Gallery style)
    ...
    // Mouse events (following D3 Gallery pattern exactly)
    const mouseOver = function(this: SVGPathElement, event: any, d: any) {
      ...
      if (countryData) {
        const value = activeTab === "credits" ? countryData.credits : countryData.projects;
        const unit = activeTab === "credits" ? "credits" : "projects";
    ...
    // Draw the map (following D3 Gallery structure)
    svg.append("g")
      .selectAll("path")
      ...
      .attr("fill", function(d: any) {
        ...
        const value = currentMetric === "credits" ? countryData.credits : countryData.projects;
